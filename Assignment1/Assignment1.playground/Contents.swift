 ...
 */
/*
 // The contents of the variable named offsets in the Grid Struct represent the 8 other areas surrounding a cell.
 ...
 */
/*
 // The word "of" is used in this function when the user inputs an argument of type Position
 */
/*:
 2. Explain in one sentence when you would use the word `position` in relation to this function
 */
/*
 // The word position is used in this function as it's what is being used internally in the function body and is equal to the value of "of", which a user inputs in.
 */
...
extension Grid {
    func neighbors(of position: Position) -> [Position] {
        return Grid.offsets.map { offset in
            ...
            return Position(norm(position.row-offset.row, to: self.rows), norm(position.col-offset.col, to: self.cols))
        }
    }
}
/*:
...
*/
/*
 // This subscript does not have a set because we cannot modify the variable "cellStates" since it is a 'let' constant and they are immutable.
 */

extension Grid {
    subscript (row: Int, col: Int) -> CellState? {
        get {
            guard self.cellStates[norm(row, to: rows)][norm(col, to: cols)] == .empty || self.cellStates[norm(row, to: rows)][norm(col, to: cols)] == .alive || self.cellStates[norm(row, to: rows)][norm(col, to: cols)] == .died || self.cellStates[norm(row, to: rows)][norm(col, to: cols)] == .born else{return nil}
            return self.cellStates[norm(row, to: rows)][norm(col, to: cols)]
        }
    }
}
 ...
 */
/*
 // Array of Tuple(Int, Int)
 */
/*:
 2. What is the type of the second argument to reduce?
 */
/*
 // A Closure in trailing closure syntax checking if an Array of Tuple(Int, Int) returns true to Boolean .isAlive and adds one to the accumulator if .isAlive returns true
 */
/*:
 ## Problem 14:
...
/*
 // The reduce function in the extension returns an Int. In the context of Conway's Game of Life, this shows how many Living Cells are on the grid.
 */
 ...
 */
/*
 // $1 refers to the second parameter that was passed into the closure, which is rows and cols.
 */
extension Grid {
    var numLiving: Int {
        return positions(rows: rows, cols: cols).reduce(0) {
...
grid.numLiving
/*
 // It should be around 33 but not necessarily exactly 33 since experimental probability is not always equal to theoretical probability, like flipping a coin and getting heads 2 times in a row, but that also doesn't mean it can't be equal to theoretical probability so it's going to be around or sometimes 33.
 */
...
extension Grid {
    func livingNeighbors(of position: Position) -> Int {
        return neighbors(of: position).reduce(0) { (count, position) in
            ...
            self[position.row , position.col] == .alive ? count + 1: count
        }
    }
}
...
extension Grid {
    func nextState(of position: Position) -> CellState {
        ...
        switch livingNeighbors(of: position) {
            case 2,3:
                if livingNeighbors(of: position) != 2 {return self[position.row,position.col]!.isAlive ? .alive : .born}
                if livingNeighbors(of: position) == 2 && self[position.row, position.col]!.isAlive {return .alive}
            default:
...
extension Grid {
    func next() -> Grid {
        ...
        return Grid(rows, cols){rows,cols in nextState /*How do I pass in row, col from the closure set in cellState? ;-; idk if i did it right*/ (of: Position(row: rows, col: cols)) }
    }
}
