...
public struct GridView: View {
    ...
    func lines(
        ...
        return Path { p in
            ...
            guard cellWidth(for: viewStore, g: g) > 5.0 else {return /* readme.md says to "simply return"... am I doing this correctly?*/ }
    ...
    func cells(
        ...
        ForEach(viewStore.grid.allOffsets, id: \.self){
            ...
            ? nil
                : GridView.self.Cell.init(offset: $0, gridSize: viewStore.grid.size, configuration: self.configuration, gridRect: g.frame(in: .local), cellState: viewStore.grid[$0.row,$0.col])
        }
        .frame(
            width: g.size.width,
...
extension GridView {
    private func convert(
        ...
        guard pos.row >= 0 && pos.row < viewStore.grid.size.rows && pos.col >= 0 && pos.col < viewStore.grid.size.cols
        else {
            return .none
        }
