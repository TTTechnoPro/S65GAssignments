    ...
    public var body: some View {
        WithViewStore(store) { viewStore in
            VStack {
                ...
                List {
                    ForEachStore(self.store.scope(state: \.configs, action: ConfigurationsState.Action.configuration(index:action:))){
                        testing in ConfigurationView(store: testing)
                        
                    }
// what goes inside the store argument???
