    ...
    Reducer<SimulationState, SimulationState.Action, SimulationEnvironment>{ state, action, env in
        switch action {
            ...
            case .grid(action:):
                return .none
            case .shrink:
                state.gridState.scaleEffect = 0.01
                    return .none
            case .expand:
                state.gridState.scaleEffect = 1.0
                    return .none
            case .showWiki:
                state.displayWiki = true
                return .none
            case .hideWiki:
                state.displayWiki = false
                return.none
            case .setIsAtStart(let value):
                state.gridState.isAtStartOfAnimation = value
                return .none
        }
    }
)
