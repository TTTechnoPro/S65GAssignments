...
struct InstrumentationView: View {
        ...
        WithViewStore(store) { viewStore in
            HStack {
                ...
                .accentColor(Color("accent"))
                    .disabled(viewStore.isRunningTimer)
    ...
    /* \(/*self.viewStore.statistics.steps*/) */
        ...
        WithViewStore(store) { viewStore in
            HStack {
                ...
                ThemedButton(text: "Step") {
                    viewStore.send(.setIsAtStart(true))
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1){
                    withAnimation(.easeOut){
                        viewStore.send(.shrink)
                        viewStore.send(.setIsAtStart(false))
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1){
    ...
    func cycleLength(for viewStore: ViewStore<SimulationState, SimulationState.Action>) -> String {
        guard let cycleLength = viewStore.gridState.history.cycleLength else {
            return "" }
        return "\(cycleLength)"
    }
