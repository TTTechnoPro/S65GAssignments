...
struct GridAnimationModifier: AnimatableModifier {
    typealias Body = GridView

    var store: Store<GridState, GridState.Action>
    var configuration: GridView.Configuration = GridView.Configuration()
...
public struct GridView: View {
    ...
    func cells(
        ...
        .offset(x: 0.0, y: 0.0)
            .scaleEffect(CGFloat(viewStore.scaleEffect))
    }
    ...
    public var body: some View {
        ...
        .rotationEffect(Angle(radians: 10.0 * Double.pi * Double(self.fractionComplete)))
        .animation(.interpolatingSpring(mass: 0.25, stiffness: 5, damping: 1 , initialVelocity: 3))
    }
}
